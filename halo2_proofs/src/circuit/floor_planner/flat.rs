...
impl FloorPlanner for FlatFloorPlanner {
    fn synthesize<F: Field, CS: Assignment<F>, C: Circuit<F>>(
        ...
        let mut constants_to_assign = Arc::try_unwrap(layouter.dynamic).unwrap().into_inner().unwrap().constants_to_assign;
        constants_to_assign.sort_by(|(_, cell0), (_, cell1)| {
            if cell0.column.index() < cell1.column.index() {
                return Ordering::Less
            } else if cell0.column.index() == cell0.column.index() {
                if cell0.row_offset < cell1.row_offset {
                    return Ordering::Less
                } else {
...
impl<'a, F: Field, CS: Assignment<F> + 'a> fmt::Debug
    for FlatChipLayouterRegion<'a, F, CS>
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("FlatChipLayouterRegion")
            .finish()
    }
}
...
impl<'a, F: Field, CS: Assignment<F> + 'a> RegionLayouter<F>
    for FlatChipLayouterRegion<'a, F, CS>
{
