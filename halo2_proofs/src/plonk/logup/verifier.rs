...
pub struct Committed<C: CurveAffine> {
    pub z_commitment_set: Vec<C>,
    pub multiplicity_commitment: MultiplicityCommitment<C>,
}

#[derive(Debug)]
pub struct ZEvalSet<C: CurveAffine> {
    pub commitment: C,
    pub eval: C::Scalar,
...
impl<F: FieldExt> Argument<F> {
    ...
    pub fn read_z_commitment<C: CurveAffine, E: EncodedChallenge<C>, T: TranscriptRead<C, E>>(
        &self,
        transcript: &mut T,
        multiplicity_commitment: MultiplicityCommitment<C>,
    ) -> Result<Committed<C>, Error> {
        let mut z_commitment_set = vec![];
        (0..self.input_expressions_sets.len()).try_for_each(|_| -> Result<_, Error> {
            z_commitment_set.push(transcript.read_point()?);
            Ok(())
        })?;
...
impl<C: CurveAffine> Committed<C> {
    pub fn evaluate<E: EncodedChallenge<C>, T: TranscriptRead<C, E>>(
        ...
        let mut z_sets = vec![];
        let mut iter = self.z_commitment_set.into_iter();
...
impl<C: CurveAffine> Evaluated<C> {
    pub(in crate::plonk) fn expressions<'a>(
        ...
        let product_expression = || {
            let first_set = self.z_sets.first().unwrap();
        ...
        let extend_product_expression =
            move |(z_set, input_express_set): (&ZEvalSet<C>, &InputExpressionSet<C::Scalar>)| {
                /*
                     φ_i(X) = f_i(X) + α
                ...
                */
                ...
                let left = z_set.next_eval - z_set.eval;
        ...
        std::iter::empty()
            .chain(
                // l_0(X) * z[0](X) = 0
                Some(l_0 * &self.z_sets.first().unwrap().eval),
            )
            .chain(
                // l_last(X) * z[last](X) = 0
                Some(l_last * &self.z_sets.last().unwrap().eval),
            )
            .chain(
                /*
                     φ_i(X) = f_i(X) + α
    ...
    pub(in crate::plonk) fn queries<'r>(
        ...
        iter::empty()
            ...
            .chain(self.z_sets.iter().flat_map(move |set| {
                iter::empty()
                    .chain(Some(VerifierQuery::new_commitment(
                        &set.commitment,
                        *x,
            ...
            .chain(self.z_sets.iter().rev().skip(1).map(move |set| {
